...
- ????????????????????????????????
- ?????? ﻿--- title: Prompt-Robust Representation Probing category: concept tags: - concept - llm - probing - robustness sources: - notes/Language Models Represent Space and Time.md created: 2026-04-08 updated: 2026-04-08 summary: 通过多提示模板一致性检验，验证探针解码结果不依赖单一 prompt 偶然性。...
- ????????????
- ??????????? URL???????????????^[ambiguous]
- ????????????arXiv / DOI / ???????????????????
- ?????????????????????
