...
SimBA 是 score-based 黑盒对抗攻击方法。它不估计完整梯度，而是沿随机基方向做“正负两次试探”，保留让攻击目标更优的更新。^[extracted]

## 基本流程

1. 选取方向 \(q\)（像素基或 DCT 基）。^[extracted]
2. 查询 \(x+\epsilon q\) 与 \(x-\epsilon q\)。^[extracted]
3. 保留更有利的一侧（例如非定向攻击里降低真类分数）。^[extracted]
4. 迭代直到成功或预算耗尽。^[extracted]
...
- 2019-05（arXiv:1905.07121）：论文直接将方法命名为 “Simple Black-box Adversarial Attacks”，核心贡献就是“简化攻击步骤 + 提升查询效率”。^[extracted]
