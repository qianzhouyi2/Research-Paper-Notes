...
SignSGD 是只使用梯度符号（而非幅值）更新参数的优化方法：
...
- 2018-10（arXiv:1810.05291）：SignSGD with Majority Vote 给出非凸情形收敛分析，并强调极低通信成本。
- 同一工作指出 majority vote 聚合对部分 Byzantine worker 具有鲁棒性，这也是其在分布式训练中的关键价值。
