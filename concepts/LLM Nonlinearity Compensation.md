...
- ????????????????????????????????
- ?????? ﻿--- title: LLM Nonlinearity Compensation category: concept tags: - concept - llm - architecture - efficiency sources: - notes/PanGu-pi Nonlinearity Compensation.md created: 2026-04-08 updated: 2026-04-08 summary: 非线性补偿通过结构增量与激活设计增强 LLM 表达能力，在较低额外成本下提升推理与任务表现。...
- ????????????
- ??????????? URL???????????????^[ambiguous]
- ????????????arXiv / DOI / ???????????????????
- ?????????????????????
