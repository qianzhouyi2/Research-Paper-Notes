...
### 2. 整体流程：先 Exploration，再 Calibration

- DLA 的总查询次数记为 `T`。
- 伪代码里把前 `T/5` 次查询分给 **Perturbation Exploration**，后 `4T/5` 次分给 **Perturbation Calibration**。
- 初始化时：
  - `l_min = L(x)`：当前最好的代理指标，从干净图像开始。
  - `δ_hat = 0`：当前保留的最好扰动。
  - `M = 1`：后面做符号翻转时用的 mask，初始全为 `+1`。
  - `i = 0, n = 0`：分别表示当前层级下的 block 编号，以及当前分块层级。

### 3. Perturbation Exploration：先找到一个“够强”的初始化

- 在前 `T/5` 次迭代里，DLA 不是累积式地一点点加噪声，而是不断在干净图像 `x` 上测试候选线性扰动，并保留效果最好的那个。
- 具体做法是交替采样横向和纵向线噪声：
  - `k = t % 2`
  - 当 `k = 1` 时采样一种方向的线噪声；
  - 当 `k = 0` 时采样另一种方向的线噪声。
- 对每个候选扰动 `δ`，都直接计算 `L(x + δ)`。
- 如果这个候选让 `mIoU` 更低，就更新：
  - `l_min = L(x + δ)`
  - `δ_hat = δ`
  - `d = k`
...
- exploration 找到 `δ_hat` 之后，DLA 不再重新随机采样整张图，而是固定住这份离散线性扰动的基本形状，只去优化它在局部区域上的符号分配。
- 这一步的核心变量是 `M`。可以把它理解成一个只取 `±1` 的翻转掩码，真正送进模型的是
  `x + δ_hat * M`
  这里是逐元素乘法。
- 如果某块区域在 `M` 里被翻成 `-1`，就相当于把该区域原本的 `+ε` 改成 `-ε`，或者把 `-ε` 改成 `+ε`。

### 5. Calibration 的分块方式到底怎么做

- DLA 不是二维网格那样同时切高和宽，而是沿 exploration 阶段选中的方向做一维层次划分。
- 如果 `d` 对应横向条纹，那么就沿高度方向切分，得到一组横向带状区域。
- 如果 `d` 对应纵向条纹，那么就沿宽度方向切分，得到一组纵向带状区域。
- 在第 `n` 层，当前 block 的尺寸写成：
  `c = ceil(axis_len / 2^n)`
  其中 `axis_len` 是当前要切分的那一维长度，也就是 `h` 或 `w`。
- 这表示：
  - `n = 0` 时，先测整条大带；
  - `n = 1` 时，切成 2 块；
  - `n = 2` 时，切成 4 块；
  - 之后继续二分，直到足够细。
- 对于当前层级的第 `i` 个 block，DLA 会先把这块区域在 `M` 里的符号整体翻转一次，然后 query 一下：
  - 如果 `L(x + δ_hat * M)` 更低，保留这次 flip；
  - 否则立刻把这块翻回去。
- 于是 calibration 其实是在做一种非常直接的贪心搜索：
  “这块带状区域整体反号，能不能让分割结果进一步变差？”

### 6. `i`、`n`、`M` 在伪代码里各自负责什么

- `i`：当前层中正在测试第几个 block。
- `n`：当前已经细化到第几层。
- `M`：当前这轮局部搜索里，已经累计接受了哪些 block flip。
- 当某一层的所有 `2^n` 个 block 都测完后：
  - `i` 归零；
  - `n = n + 1`；
  - 进入更细一层继续翻。
- 当层级已经细到
  `ceil(log2(axis_len)) + 1`
  时，就认为这一轮分层校准走完了。
- 伪代码随后把当前有效的翻转结果并入 `δ_hat`，再把 `i`、`n` 重新置零，开始下一轮更高层面的校准。
- 也就是说，DLA 不是“一次 coarse-to-fine 就结束”，而是在剩余 query 允许的范围内，反复做这种分层翻转搜索。

### 7. 把伪代码翻成更容易实现的流程

1. 输入干净图像 `x`、真值标签 `y`、模型 `f`、扰动预算 `ε`、总 query `T`。
2. 定义代理指标 `L(x') = mIoU(f(x'), y)`。
3. 用前 `T/5` 次 query 交替测试 horizontal / vertical 的离散线噪声，选出让 `L` 最小的 `δ_hat`，并记录方向 `d`。
4. 用剩余 query 对 `δ_hat` 做 coarse-to-fine 的 block sign flip：
   - 先测大块；
   - 再测更细的块；
   - 每次只保留能进一步降低 `mIoU` 的 flip。
5. 输出最终对抗样本 `x_adv = x + δ_hat`（严格按伪代码理解，是合并了当前最佳 flip mask 之后的 `δ_hat`）。
