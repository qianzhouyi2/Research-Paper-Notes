---
title: Yejin Choi
category: entity
tags:
- entity
- author
- llm
- reasoning
sources:
- notes/From Explicit CoT to Implicit CoT Learning to Internalize CoT Step by Step.md
created: 2026-04-08
updated: 2026-04-09
summary: Yejin Choi 在该 wiki 主要关联 CoT 内化与推理训练方法研究。
provenance:
  extracted: 0.81
  inferred: 0.17
  ambiguous: 0.02
---

# Yejin Choi

## 联网补充

- 根据《From Explicit CoT to Implicit CoT Learning to Internalize CoT Step by Step》的一手作者列表，Yejin Choi 是该文共同作者之一；这篇工作主要围绕 通过逐步移除显式链式思维训练隐式推理。
- 从当前入库范围看，Yejin Choi 在本库里首先与 结构化推理 这条研究线相连，说明已读材料对其关注面目前集中在这里。

## 关联页面

- [[references/From Explicit CoT to Implicit CoT Learning to Internalize CoT Step by Step]]
- [[concepts/Removal Smoothing for CoT Internalization]]
- [[synthesis/Process Supervision and CoT Internalization]]

