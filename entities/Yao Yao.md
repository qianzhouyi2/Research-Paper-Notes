...
- ?????????????????????????????
- ?????? - 根据《Beyond Chain-of-Thought, Effective Graph-of-Thought Reasoning in Language Models》的一手作者列表，Yao Yao 是该文共同作者之一；这篇工作主要围绕 把图结构推理扩展到比线性 CoT 更复杂的问题求解。 - 从当前入库范围看，Yao Yao 在本库里首先与 结构化推理 这条研究线相连，说明已读材料对其关注面目前集中在这里。
- ????????????
- ??????????? URL???????????????^[ambiguous]
- ????????????arXiv / DOI / ???????????????????
- ?????????????????????
