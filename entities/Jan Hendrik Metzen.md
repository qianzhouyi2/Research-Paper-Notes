---
title: "Jan Hendrik Metzen"
category: entity
tags:
  - entity
  - author
  - semantic-segmentation
  - adversarial-robustness
sources:
  - papers_sources/semantic_segmentation_robustness_20260409/download_report.json
  - https://arxiv.org/abs/2010.05495
  - https://arxiv.org/abs/2209.05980
created: 2026-04-10
updated: 2026-04-10
summary: "Jan Hendrik Metzen is an author entity linked to the verified semantic-segmentation robustness corpus."
provenance:
  extracted: 0.81
  inferred: 0.16
  ambiguous: 0.03
---

# Jan Hendrik Metzen

## Role in This Corpus

- Appears in 2 verified paper(s) from the segmentation robustness corpus.
- Primary context: adversarial attacks, robust training, certification, or reliability benchmarking. ^[inferred]

## Linked Papers

- [[references/Increasing the Robustness of Semantic Segmentation Models with Painting-by-Numbers|Increasing the Robustness of Semantic Segmentation Models with Painting-by-Numbers]]
- [[references/Certified Defences Against Adversarial Patch Attacks on Semantic Segmentation|Certified Defences Against Adversarial Patch Attacks on Semantic Segmentation]]

## Online Verification Notes

- Author attribution is grounded in official paper metadata (arXiv or Crossref) and aligned with local artifacts.

## Links

- [[synthesis/Semantic Segmentation Robustness Corpus 2019-2026]]
- [[concepts/Segmentation Robustness Benchmark Protocol]]

## Online Supplement (2026-04-10)

- This entity page is cross-checked online for attribution and role consistency across linked papers.
- Text anchor used: - Appears in 2 verified paper(s) from the segmentation robustness corpus. - Primary context: adversarial attacks, robust training, certification, or reliability benchmarking. ^[inferred]
- Primary online sources used in this pass:
- https://arxiv.org/abs/2010.05495
- https://arxiv.org/abs/2209.05980
- Policy: prioritize primary sources (arXiv/DOI/official venue pages) and preserve ambiguity markers for unresolved conflicts.
- Status: completed page-level online supplementation in this global pass.

