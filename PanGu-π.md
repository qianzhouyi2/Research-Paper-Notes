...
Transformer模型 [13]是许多LLM的基础架构。Transformer模型的核心组件包括多头自注意力（MSA）和前馈网络（FFN）。MSA为输入序列中的每个标记计算与所有其他标记的注意力分数，捕捉关系和依赖性。 FFN单独对每个标记执行操作，提供更多的非线性变换和模型容量。
...
以LLaMA为例， 使用秩1矩阵 （rank-1 metric）[19]对其特征崩溃现象进行了分析。
