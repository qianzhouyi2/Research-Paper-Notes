...
将这种类型的算子称为$\textit{数据控制}$ 型，因为它们通过 $u$ 非线性地定义了一个线性变换 $u \mapsto y$。这种方法在 $u$ 中产生了具有表达力的非线性算子，并且假设，与其他机制(induction heads)一起，这有助于某些算子能够通过利用上下文从而学习到$\textit{上下文中的知识}$，从而适应从未见过的任务。
