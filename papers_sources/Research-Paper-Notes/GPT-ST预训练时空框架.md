...
>  渲染：渲染来自于计算机图形学。渲染过程中我们需要提供几何信息，如告诉计算机在何处绘制对象，比如对象的比例、角度、空间信息等。
>  逆渲染：渲染结果反推出物体的信息，包括空间几何信息
...
>  胶囊内的操作
>  传统神经网络：加权求和+非线性激活
>  胶囊网络： 1. 输入向量 x 权重矩阵：编码了低级特征和高级特征的空间关系 2. 加权输入向量：权重决定当前胶囊将其输出到哪个更高级的胶囊。通过动态路由实现。 3. 加权后求和 4. 非线性激活，使用squash函数：将向量进行压缩使其长度在0-1之间，方向保持不变。
>  ![[GPT-ST/Pasted image 20240410123511.png]]
...
聚类感知掩码机制算法：
输入：ST 数据 X  ，最大历元数 E，总掩模比 $r_t$，X 元素数量 J
输出：掩码矩阵 M
1.双层MLP网络，随后使用一个线性层和$\text{softmax}(\cdot)$函数来获得预测值$q_{i,r,t}\in\mathbb{R}$
2.计算自适应掩码率  $r_a$
3.计算掩码数量 $m_t$ ,自适应掩码数 $m_a$ ,随机掩码数 $m_r$
4.从分类列表中随机选择n个类别，直到这些类别的元素总数大于$m_a$。掩码前 n − 1 个类别的所有元素，并随机掩码第 n 个类别的元素，剩余掩码数为 $m_a$
5.随机掩码 $m_r$ 个剩余元素
## 5 评估
...
![[GPT-ST/Pasted image 20240409174009.png]]5.将自定义参数 $d_t$ 和 $c_r$ 集成到时序超图神经网络中，用Γ对时序交通模式进行编码
